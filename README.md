...
header row) and rows are iterated to build and store the specifications.

    specs_lista    <- list()
    outliers_lista <- list()

    for (indicador in hojas_ind) {
      df <- read_excel(ruta_modelos, sheet = indicador, skip = 1, col_names = TRUE)
      ...
      for (i in seq_len(nrow(df))) {
        ...
        fila  <- as.list(df[i, ])
        clave <- paste0(ccaa, ".", indicador)   # e.g. "AND.AFI"
...
Keys in `specs_lista` follow the format `"<CCAA>.<INDICATOR>"`
(e.g. `"AND.AFI"`, `"CAT.IPI"`). Failed specifications are stored as
`NULL` without interrupting the loop.

A separate `outliers_lista` is also populated with manual outliers in a
legacy format (vectors of types and dates), maintaining compatibility
with other functions in the project.
...
were successfully built per indicator and which ones failed:
    ...
    for (ind in hojas_ind) {
      ...
      # Prints: indicator | OK: N | ERROR: M -> failed keys
    }
...
Example console output:
    ...
    === SPEC BUILD SUMMARY ===
      AFI    OK: 19
      IPI    OK: 17  |  ERROR: 2  ->  CEU.IPI, MEL.IPI
      EPA    OK: 19
      -----------------------------------
      TOTAL  OK: 55  |  ERROR: 2
